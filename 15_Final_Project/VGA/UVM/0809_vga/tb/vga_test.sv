...
class vga_test_random extends uvm_test;
    `uvm_component_utils(vga_test_random)
    vga_env env;
...
class vga_test_testImage extends uvm_test;
    `uvm_component_utils(vga_test_testImage)
    vga_env env;
    ...
    virtual task run_phase(uvm_phase phase);
        vga_testImage_seq seq;

        phase.raise_objection(this);
        phase.phase_done.set_drain_time(this, 2000ns);
        seq = vga_testImage_seq::type_id::create("seq");
        seq.start(env.agt.sqr);
        phase.drop_objection(this);
    endtask
