...
void setup() {
  ...
  // 2. Basys 3 수신용 HardwareSerial2 (RX2=GPIO 4, TX2=GPIO 17)
...
void loop() {
  ...
  // 2. Basys 3로부터 UART 데이터가 2바이트 이상 수신되었는지 확인
  if (Serial2.available() >= 2) {
    ...
    /* 참고: 만약 Jetson에서 바이너리가 아닌 읽기 쉬운 ASCII 문자열로 받는 구조라면 
       아래 줄의 주석을 풀고 위 write() 대신 사용하세요.
       client.printf("Data: 0x%04X (%d)\n", combined_data, combined_data);
    */
