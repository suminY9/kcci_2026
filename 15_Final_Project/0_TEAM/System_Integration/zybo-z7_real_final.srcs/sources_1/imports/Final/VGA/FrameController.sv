module FrameController(
    ...
    // ila_framectrl_debug U_ILA_FRAMECTRL_DEBUG (
    //     .clk   (i_pixel_clk),
    //     .probe0(i_vga_start),
    //     .probe1(state),
    //     .probe2(n_state),
...
module FrameMono(
    ...
    // binary filter
    always_comb begin
        if(gray >= 8'b1100_0000) o_pdata = 1'b0;
        else                     o_pdata = 1'b1;
    end
endmodule
...
module FrameRegister(
    ...
    assign o_debug_line_cnt = lineCnt;
    assign o_debug_bit_cnt  = bitCnt;
    assign o_debug_pos_cnt  = posCnt;
