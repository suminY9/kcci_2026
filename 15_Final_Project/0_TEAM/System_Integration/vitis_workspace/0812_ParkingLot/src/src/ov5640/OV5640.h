...
namespace OV5640_cfg {
	...
	using mode_t = enum { MODE_720P_1280_720_60fps = 0, MODE_1080P_1920_1080_15fps,
		...
		MODE_1080P_1920_1080_30fps_336M_1LANE_MIPI, MODE_VGA_640_480_60fps, MODE_VGA_640_480_30fps, MODE_END } ;
	...
	{
	    // 640 x 480, RAW10, 2-lane MIPI
	    // MIPI clock = 240 MHz
	    // System/Pixel processing clock = 48 MHz

	    // PLL configuration
	    // System clock divider /2, MIPI scale divider /1
	    {0x3035, 0x21},

	    // PLL multiplier = 60
	    {0x3036, 0x3C},

	    // PLL root divider /1, PLL pre-divider /1.5
	    {0x3037, 0x05},

	    // PCLK root divider /2, SCLK root divider /2
	    {0x3108, 0x11},
	     ...
	     * Frame timing
	     *
	     * 48 MHz / (1600 ¡¿ 500) = 60 frames/sec
	     */

	    // HTS = 1600 clocks per line
	    {0x380C, (1600 >> 8) & 0x1F},
	    {0x380D, 1600 & 0xFF},

	    // VTS = 500 lines per frame
	    {0x380E, (500 >> 8) & 0xFF},
	    {0x380F, 500 & 0xFF},
	...
	config_word_t const cfg_vga_30fps_[] =
		{
		    // 640 x 480, RAW10, 2-lane MIPI
		    // MIPI clock = 240 MHz
		    // System/Pixel processing clock = 48 MHz
			 ...
			 * Frame timing
			 *
			 * 48 MHz / (1600 * 1000) = 30 frames/sec
			 */
...
config_word_t const cfg_1080p_30fps_336M_1lane_mipi_[] =
	...
	config_modes_t const modes[] =
	{
			...
			{ MAP_ENUM_TO_CFG(MODE_VGA_640_480_30fps, cfg_vga_30fps_) },
	};
