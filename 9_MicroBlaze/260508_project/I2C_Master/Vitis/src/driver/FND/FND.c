...
void FND_Init() {
	GPIO_SetMode(FND_DIGIT, FND_COM_DIG_1|FND_COM_DIG_2|FND_COM_DIG_3|FND_COM_DIG_4, OUTPUT);
	GPIO_SetMode(FND_DATA, SEG_PIN_A|SEG_PIN_B|SEG_PIN_C|SEG_PIN_D|SEG_PIN_E|SEG_PIN_F|SEG_PIN_G|SEG_PIN_DP, OUTPUT);
}
...
void FND_DispDP(uint8_t digit){
	if(fndDpData & (1<<0)){
		GPIO_WritePin(FND_DIGIT, GPIO_PIN_7, RESET);
	}
	else {
		GPIO_WritePin(FND_DIGIT, GPIO_PIN_7, SET);
	}
}
