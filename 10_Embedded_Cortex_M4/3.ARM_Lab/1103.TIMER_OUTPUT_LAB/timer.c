...
void TIM3_Out_Freq_Generation(unsigned short freq)
{
	...
	TIM3->PSC = (unsigned int)(TIMXCLK / (double)TIM4_FREQ + 0.5) - 1; // 더 정밀한 계산을 위해 double로 형변환
	// 요청한 주파수가 되도록 ARR 설정
	TIM3->ARR = (unsigned int)(TIM3_FREQ / (double)freq) - 1;
	// Duty Rate 50%가 되도록 CCR3 설정
	TIM3->CCR3 = (unsigned int)(TIM3->ARR / 2);
	// Manual Update(UG 발생)
	Macro_Set_Bit(TIM3->EGR, 0);
	// Down Counter, Repeat Mode, Timer Start
	TIM3->CR1 = (0 << 7) | (1 << 4) | (0 << 3) | ( 0 << 1) | (1 << 0);
}
