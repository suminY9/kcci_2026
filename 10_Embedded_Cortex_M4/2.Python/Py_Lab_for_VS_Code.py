...
# def string_repeat(s: str, n: int) -> str :
...
# 코드 작성
# y = ('*'+n[1:] for n in input().split() if n[0] == 'a')
...
# # l = [x for x in (int(s) for s in input().split()) if x >= 10] # map은 function이기 때문에 generator가 더 빠름
...
# # 코드 작성
# l = [x*2 for x in (int(i) for i in input().split()) if x >= 10]
# print(*l)
...
# #코드 작성
# def pay(time: int) -> int :
#     if time <  10 : return 0
#     if 10 <= time < 30 : return 500
#     if time >= 30 :
#         fee = int(500 + ((time-30)//10)*300)
#         if time % 30 : fee += 300
#         if fee >= 50000 : return 50000
#         else : return fee
...
# #     for i in range(len(t)):   # column index
# #         print(t[i][j], end=' ')
...
#     # 코드 작성
#     l = sorted(set(range(1, 1+N)) - set(r))
#     print(*l)
